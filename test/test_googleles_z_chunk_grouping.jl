...
function _union_chunk_hull_z(spans::Vector{UnitRange{Int}}, nz::Int, cz::Int)::UnitRange{Int}
    ...
    cr = Tuple{Int, Int}[M._z_native_span_to_storage_chunk_range(first(s), last(s), cz) for s in spans]
    cmin = Statistics.minimum(x[1] for x in cr)
    cmax = Statistics.maximum(x[2] for x in cr)
    return M._z_storage_chunk_hull_to_native_layer_range(cmin, cmax, cz, nz)
end
...
Test.@testset "GoogleLES z-chunk span grouping" begin
    ...
    spans = [1:10, 40:55]
    g = M._group_mask_spans_by_overlapping_z_chunks(spans, nz, cz)
    ...
    spans2 = [1:10, 305:310]
    g2 = M._group_mask_spans_by_overlapping_z_chunks(spans2, nz, cz)
    ...
    spans3 = [1:60, 61:70]
    g3 = M._group_mask_spans_by_overlapping_z_chunks(spans3, nz, cz)
    Test.@test length(g3) == 2

    # Two mask spans in the same storage chunk (3)
    spans5 = [121:130, 150:160]
    g5 = M._group_mask_spans_by_overlapping_z_chunks(spans5, nz, cz)
