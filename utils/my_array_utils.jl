#=
    ...
    - `utils/dataset_builder_impl.jl`: set `spatial_info.coarsening_mode = :convolutional` (or env
      `MLCD_COARSENING_MODE=convolutional` via `GoogleLES.build_tabular` / cfSites builder).
