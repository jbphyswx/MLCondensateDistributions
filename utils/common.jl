...
export robust_limits, is_constant, centers_to_edges, binned_median, write_metrics_table
...
"""
function binned_median(x::AbstractVector{<:Real}, y::AbstractVector{<:Real}; nbins::Int=30)
    finite = isfinite.(x) .& isfinite.(y)
    xf = Float64.(x[finite])
    yf = Float64.(y[finite])
    isempty(xf) && return Float64[], Float64[]

    xlo, xhi = robust_limits(xf; qlo=0.01, qhi=0.99)
    if xlo == xhi
