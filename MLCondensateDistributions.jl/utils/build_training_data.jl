...
function _googleles_tabular_one_span!(
    ...
    df = DatasetBuilder.process_abstract_chunk(fine_fields, metadata_t, spatial_info_t, max_dz)
    if DataFrames.nrow(df) > 0
        if !skip_finite_assert_per_chunk
...
"""
    GoogleLES.build_tabular(site_id::Int, month::Int, experiment::String, output_dir::String; 
                               ...
                               min_dh::Float32=1000.0f0, verbose::Bool=false)
...
5. **Non-`q_c` Zarr reads**: By default `nonqc_single_fused_load=true` in [`TabularBuildOptions`](@ref),
   all mask spans for the timestep are passed as **one** group so **`_load_googleles_timestep_fields_into_span_list!`**
   runs **once** per cloudy timestep (fused z-indices). Set `nonqc_single_fused_load=false` for chunk-merge
   (`z_chunk_merge`) or per-span groups. Reads stay **narrow** unless `nonqc_strategy` is `full_timestep`.
...
- `output_dir::String`: Directory where parquet output file is written
...
- `min_dh::Float32=1000.0f0`: Minimum horizontal grid resolution to extract
- `verbose::Bool=false`: Enable verbose progress reporting
- `tabular_options::TabularBuildOptions=TabularBuildOptions()`: I/O, coarsening, and profiling knobs (see [`TabularBuildOptions`](@ref)).

# Output

Writes a parquet file `<output_dir>/googleles_case__<site_id>__month__<month>__exp__<experiment>.parquet` containing:
...
- **Options**: pass [`TabularBuildOptions`](@ref) as `tabular_options` (defaults match previous behavior). For shell/CLI, call [`tabular_build_options_from_env`](@ref) once and pass the result so `MLCD_*` env vars are not read inside the hot path.
- Non-`q_c` loads: `nonqc_single_fused_load` (default `true`) → one fused read per cloudy timestep; set `false` with `z_chunk_merge` / per-span groups. Full column: `nonqc_strategy=\"full_timestep\"`.
...
- Writes empty parquet file with schema if no trainable rows extracted
- Validates all finite values before writing

"""
function GoogleLES.build_tabular(
    site_id::Int,
    month::Int,
    experiment::String,
    output_dir::String;
    max_timesteps::Int=0,
    timestep_batch_size::Int=0,
    min_dh::Float32=GoogleLES.GoogleLESDefaults.min_horizontal_averaging_domain,
    max_dz::Float32=GoogleLES.GoogleLESDefaults.max_vertical_averaging_domain,
    verbose::Bool=false,
    ...
    fullcase_bytes_limit = opts.fullcase_bytes_limit
    force_fullcase = opts.force_fullcase
    ...
    z_schemes_native = CoarseGraining.compute_z_coarsening_scheme(spatial_info.dz_native_profile, max_dz)
    ...
    merge_z_chunks = opts.z_chunk_merge
    nonqc_single_fused_load = opts.nonqc_single_fused_load
    if verbose
    ...
    cache_seconds = 0.0
    timestep_prof = opts.timestep_profile
    prof_each = opts.timestep_profile_each
    ...
    if is_full_case
        ...
        for local_t in 1:nt
            ...
            load_full_nonqc = _googleles_use_full_nonqc_timestep_load(n_spans_z, n_keep_z, nz_z; nonqc_strategy=opts.nonqc_strategy)
            ...
            if load_full_nonqc
                ...
                for z_range in spans
                    t_tabular += @elapsed _googleles_tabular_one_span!(
                        ...
                        case_df_acc,
                        t_idx;
                        skip_finite_assert_per_chunk=opts.skip_finite_assert_per_chunk,
                    )
                end
            else
                for orig_spans in span_groups
                    t_nonqc += @elapsed _load_googleles_timestep_fields_into_span_list!(
                        ...
                        fuse_span_z_reads=opts.fuse_span_z_reads,
                    )
    ...
    if isnothing(final_df)
        out_file = case_parquet_path(site_id, month, experiment, output_dir)
        Parquet2.writefile(out_file, _empty_googleles_case_df(); compression_codec=:snappy)
        if verbose
            @info "No trainable GoogleLES rows found for site=$site_id month=$month experiment=$experiment; wrote empty parquet case file."
        end
        _safe_close_http_pools!(; close_http_pools=opts.close_http_pools)
        return
    end

    out_file = case_parquet_path(site_id, month, experiment, output_dir)
    _assert_finite_dataframe(final_df, "GoogleLES final site=$(site_id) month=$(month) experiment=$(experiment)")
    t_arrow = @elapsed Parquet2.writefile(out_file, final_df; compression_codec=:snappy)
    println("Wrote GoogleLES case file $(out_file) with $(DataFrames.nrow(final_df)) rows.")
    if timestep_prof
        println("GoogleLES parquet write wall time: $(round(t_arrow; digits=3))s")
    end
    _safe_close_http_pools!(; close_http_pools=opts.close_http_pools)
end

"""
    cfSites.build_tabular(cfSite_number, month, forcing_model, experiment, output_dir;
                          max_timesteps=0, min_dh=1000f0, verbose=false,
                          tabular_options=TabularBuildOptions())

Local netCDF → parquet tabular build. Uses `tabular_options.coarsening_mode` and
`tabular_options.skip_finite_assert_per_chunk`; other GoogleLES-specific options are ignored.
"""
function cfSites.build_tabular(
    cfSite_number::Int,
    month::Int,
    ...
    output_dir::String;
    max_timesteps::Int=0,
    min_dh::Float32=cfSites.cfSitesDefaults.min_horizontal_averaging_domain,
    max_dz::Float32=cfSites.cfSitesDefaults.max_vertical_averaging_domain,
    verbose::Bool=false,
    tabular_options::TabularBuildOptions=TabularBuildOptions(),
)
    opts = tabular_options
    ...
    for t_idx in 1:nt
        ...
        df = DatasetBuilder.process_abstract_chunk(fine_fields, metadata_t, spatial_info, max_dz)
        if size(df, 1) > 0
            if !opts.skip_finite_assert_per_chunk
    ...
    if isnothing(final_df)
        if verbose
            @info "No trainable cfSites rows found for site=$cfSite_number month=$month model=$forcing_model experiment=$experiment; no parquet file written."
        end
        return
    end

    out_file = joinpath(output_dir, "cfsites_$(forcing_model)_$(experiment)_$(month)_$(cfSite_number).parquet")
    _assert_finite_dataframe(final_df, "cfSites final site=$(cfSite_number) month=$(month) model=$(forcing_model) experiment=$(experiment)")
    Parquet2.writefile(out_file, final_df; compression_codec=:snappy)
