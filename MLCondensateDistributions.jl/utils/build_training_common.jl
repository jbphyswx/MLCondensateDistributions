...
Base.@kwdef struct TabularBuildOptions
    ...
    """Total horizontal budget (default 10): split into lower-band block-seed count and upper-band hybrid sliding subsample (`ReductionSpecs.horizontal_sliding_budget_split`). Also sets vertical tower/extra budgets when building triples."""
    sliding_window_budget_h::Int = 10
end
...
| `sliding_window_budget_h` | `MLCD_SLIDING_WINDOW_BUDGET_H` (default 10: lower block seeds + upper hybrid sliding, see `ReductionSpecs.horizontal_sliding_budget_split`; vertical tower uses same total) |
"""
function tabular_build_options_from_env(; kw...)::TabularBuildOptions
    ...
    base = (;
        ...
        sliding_window_budget_h = max(1, parse(Int, get(ENV, "MLCD_SLIDING_WINDOW_BUDGET_H", "10"))),
    )
