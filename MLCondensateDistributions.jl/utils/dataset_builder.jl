...
"""
    process_abstract_chunk(fine_fields, metadata, spatial_info, max_dz)

Default dataset-builder entrypoint.
This routes to the current implementation in `DatasetBuilderImpl`.
"""
@inline function process_abstract_chunk(fine_fields, metadata, spatial_info, max_dz)
    return DatasetBuilderImpl.process_abstract_chunk_impl(fine_fields, metadata, spatial_info, max_dz)
end
