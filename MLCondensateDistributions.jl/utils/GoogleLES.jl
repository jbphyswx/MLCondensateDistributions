module GoogleLES
    #=
        ...
        #   p, p_ref       : Hydrodynamic perturbation pressure and reference pressure (Pa).
...
using SwirlLMCloudBench: Catalog, Simulation



const GoogleLESDefaults = (;
    cases = Catalog.CLOUDBENCH_CASE_INDICES,
    months = Catalog.CLOUDBENCH_MONTHS,
    experiments = Val.(Catalog.EXPERIMENTS),
    min_horizontal_averaging_domain = Float32(200),
    max_vertical_averaging_domain = Float32(400),
)
#=
    amip: present-day climate (baseline).
    amip-p4k: 4 K SST increase relative to the baseline.
    amip-4xco2: quadrupled atmospheric CO2 concentration relative to the baseline.
    amip-p4k-2xco2: 4 K warming with doubled CO2 concentration relative to the baseline.
    amip-p4k-4xco2: 4 K warming with quadrupled CO2 concentration relative to the baseline.
=#
GoogleLES_experiment_string(exp::Catalog.CloudBenchExperiment) = Catalog.gcs_path_segment(exp)
GoogleLES_experiment_string(exp::Symbol) = Catalog.gcs_path_segment(exp)
GoogleLES_experiment_string(exp::String) = Catalog.gcs_path_segment(exp)
...
const GoogleLES_to_CliMAVars_translation = Dict{String, String}(
    ...
    "p_ref"    => "pfull",      # Canonical pressure feature for GoogleLES (reference pressure)
...
"""
@inline liquid_frac(T::FT) where {FT} = Simulation.condensate_liquid_fraction(T)
...
@inline function partition_condensate(q_c::FT, T::FT) where {FT}
    return Simulation.split_q_c(q_c, T)
end
...
function load_zarr_simulation(site_id::Int, month::Int, experiment::String="amip")
    parsed = Catalog.CloudBenchExperiment(experiment)
    return Simulation.open_zarr(site_id, month, parsed)
end
