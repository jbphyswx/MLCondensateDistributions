...
"""
    prune_incompatible_parquet_files!(data_dir::String, required_cols::Vector{Symbol}; verbose::Bool=true)

Delete parquet files in `data_dir` that are empty or do not contain all `required_cols`.
Returns a vector of compatible file paths.
"""
function prune_incompatible_parquet_files!(data_dir::String, required_cols::Vector{Symbol}; verbose::Bool=true)
    all_files = filter(f -> endswith(f, ".parquet"), readdir(data_dir, join=true))
    compatible_files = String[]

    for f in all_files
        table = Parquet2.Dataset(f)
        df = DataFrames.DataFrame(table)

        if DataFrames.ncol(df) == 0 || DataFrames.nrow(df) == 0
            rm(f; force=true)
            if verbose
                @warn "Removed empty parquet file" file=f
            end
            continue
        end
        ...
        if !all(c -> c in file_cols, required_cols)
            ...
            rm(f; force=true)
            if verbose
                @warn "Removed incompatible parquet file with missing training columns" file=f missing=missing_cols
            end
            continue
        end
        ...
        if has_nonfinite
            rm(f; force=true)
            if verbose
                @warn "Removed parquet file containing non-finite numeric values" file=f
            end
            continue
        end
...
Loads all .parquet files from a directory and returns (features, targets) as matrices.
"""
function load_processed_data(data_dir::String, feature_cols::Vector{Symbol}, target_cols::Vector{Symbol})
    ...
    compatible_files = prune_incompatible_parquet_files!(data_dir, required_cols)
    
    if isempty(compatible_files)
        error("No .parquet files found in $data_dir. Please run utils/build_training_data.jl first.")
    end
    ...
    if isempty(dfs)
        error("No compatible non-empty .parquet files in $data_dir. Ensure processed files include all required training columns.")
    end
...
Print schema, shape, and the first `n` rows from a processed parquet file.
Returns the loaded DataFrame.
"""
function preview_processed_file(file_path::String; n::Int=10)
    df = DataFrames.DataFrame(Parquet2.Dataset(file_path))
