...
"""
        process_abstract_chunk_impl(fine_fields, metadata, spatial_info, max_dz::FT) where {FT <: Real}
...
- `spatial_info`: grid metadata. Required fields: `dx_native`, `domain_h`,
    `dz_native_profile`. Optional: `min_dh`, `coarsening_mode` (`:hybrid` default,
...
function process_abstract_chunk_impl(
    ...
    min_dh = FT(_spatial_lookup(spatial_info, :min_dh, 1000.0f0))
...
function _process_abstract_chunk_block_truncated(
    ...
    h_budget = Int(_spatial_lookup(spatial_info, :sliding_window_budget_h, 10))
...
function _process_abstract_chunk_sliding(
    ...
    win_budget = Int(_spatial_lookup(spatial_info, :sliding_window_budget_h, 10))
    dz_ref = FT(sum(dz_native) / length(dz_native))
...
function _process_abstract_chunk_hybrid(
    ...
    _, b_hi = ReductionSpecs.horizontal_sliding_budget_split(win_budget)
