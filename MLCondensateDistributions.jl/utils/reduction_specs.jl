...
export AbstractReductionSpec,
    ...
    horizontal_block_factors_from_tower,
    horizontal_sliding_budget_split,
    effective_fz_max,
...
Schedule is built from `min_h`, `max_dz`, and native grid — see [`horizontal_block_factors_from_tower`](@ref) / [`vertical_block_factors_from_tower`](@ref).
"""
...
function _horizontal_tower_dfs!(out::Set{Int}, nh_eff::Int, nxc::Int, nyc::Int, nh_cap::Int)
    min_side = min(nxc, nyc)
    min_side < 2 && return nothing
    for p in _primes_asc_up_to(min_side)
        div(nxc, p) < 1 && continue
        div(nyc, p) < 1 && continue
        nh_child = nh_eff * p
        nh_child > nh_cap && continue
        push!(out, nh_child)
        nxc_child = div(nxc, p)
        nyc_child = div(nyc, p)
        _horizontal_tower_dfs!(out, nh_child, nxc_child, nyc_child, nh_cap)
    end
    return nothing
end

"""
    horizontal_sliding_budget_split(B) -> (b_lo, b_hi)

Split total horizontal budget `B` (default knob `sliding_window_budget_h`) into:
- **`b_lo`**: subsample count for **block tower seeds** in `[nh_min, ⌊N/2⌋]` (or full single-band fallback).
- **`b_hi`**: subsample count for **hybrid sliding** candidates in `[max(nh_min, ⌊N/2⌋), N]`.

For `B == 1`, `b_hi == 0` (no upper-band sliding subsample).
"""
function horizontal_sliding_budget_split(B::Int)
    B >= 1 || throw(ArgumentError("horizontal budget B must be >= 1, got $B"))
    b_lo = max(1, div(B, 2))
    b_hi = B - b_lo
    return (b_lo, b_hi)
end
...
"""
    horizontal_block_factors_from_tower(nx, ny, dx, min_h; horizontal_budget=10)

Horizontal **block** footprints `nh` (native cells per coarse horizontal cell, square `nh×nh`), per
`docs/TABULAR_REDUCTION_SCHEDULE_SPEC.md` §3: seeds subsampled in **`[nh_min, ⌊N/2⌋]`** (with §3.1
fallback when `nh_min > ⌊N/2⌋`), then **prime `p×p`** macro-pool DFS on successive coarse grids until
`nh · p` would exceed **`N = min(nx, ny)`** or tiling fails.

- **`horizontal_budget === nothing`:** seed list is every integer in the seed band (no subsampling);
  tower enumeration is unchanged.
- **`horizontal_budget` an `Int ≥ 1`:** uses [`horizontal_sliding_budget_split`](@ref) — **`b_lo`**
  subsamples seeds only; tower may still emit many distinct `nh` (including `nh > ⌊N/2⌋`).

See §3.3: “lower band” refers to **where seeds are chosen**, not that all block outputs satisfy `nh ≤ N/2`.
"""
function horizontal_block_factors_from_tower(
    nx::Int,
    ny::Int,
    dx::T,
    min_h::T;
    horizontal_budget::Union{Nothing,Int} = 10,
) where {T <: Real}
    nh_cap = min(nx, ny)
    nh_min = max(1, ceil(Int, min_h / dx))
    nh_half = fld(nh_cap, 2)
    nh_min > nh_cap && return Int[nh_cap]
    out = Set{Int}()
    seeds = if nh_min > nh_half
        if horizontal_budget === nothing
            collect(nh_min:nh_cap)
        else
            horizontal_budget < 1 && throw(ArgumentError("horizontal_budget must be ≥ 1 or nothing"))
            subsample_closed_range(nh_min, nh_cap, horizontal_budget)
        end
    else
        if horizontal_budget === nothing
            collect(nh_min:nh_half)
        else
            horizontal_budget < 1 && throw(ArgumentError("horizontal_budget must be ≥ 1 or nothing"))
            b_lo, _ = horizontal_sliding_budget_split(horizontal_budget)
            subsample_closed_range(nh_min, nh_half, b_lo)
        end
    end
    isempty(seeds) && push!(seeds, nh_min)
    for nh0 in seeds
        nh0 < 1 && continue
        nh0 > nh_cap && continue
        nx_used = div(nx, nh0) * nh0
        ny_used = div(ny, nh0) * nh0
        nxc = div(nx_used, nh0)
        nyc = div(ny_used, nh0)
        (nxc >= 1 && nyc >= 1) || continue
        push!(out, nh0)
        _horizontal_tower_dfs!(out, nh0, nxc, nyc, nh_cap)
    end
...
"""
    block_reduction_triples(nx, ny, nz, dx, min_h, dz_ref, max_dz; square_horizontal=true, horizontal_budget=10)

All `(nh, nh, fz)` where `nh` comes from [`horizontal_block_factors_from_tower`](@ref) (§3 seeds + prime
horizontal tower) and each `fz` comes from [`vertical_block_factors_from_tower`](@ref) with the same
`vertical_budget = horizontal_budget`.
"""
function block_reduction_triples(
    ...
    nhs = horizontal_block_factors_from_tower(nx, ny, dx, min_h; horizontal_budget)
...
"""
    sliding_reduction_triples(nx, ny, nz, dx, min_h, dz_ref, max_dz; horizontal_budget=10)
...
Horizontal window sizes for hybrid **sliding** passes (TABULAR §4): up to `budget` subsampled widths in
**`[max(nh_min, ⌊N/2⌋), N]`** that are **not** in `block_nhs`. If that yields nothing and the **upper band**
is not fully contained in `block_nhs`, falls back to [`default_hybrid_sliding_windows`](@ref) filtered the
same way. If `budget < 1`, returns `Int[]` (caller may pass `b_hi == 0` when total horizontal budget is 1).
"""
function hybrid_sliding_extra_sizes_default(
    ...
    nh_min = max(1, ceil(Int, min_h / dx))
    nh_max = min(nx, ny)
    nh_half = fld(nh_max, 2)
    nh_slide_lo = max(nh_min, nh_half)
    nh_slide_hi = nh_max
    cand = if budget < 1 || nh_slide_lo > nh_slide_hi
        Int[]
    else
        subsample_closed_range(nh_slide_lo, nh_slide_hi, budget)
    end
    extra = filter(w -> !(w in block_nhs), cand)
    if !isempty(extra)
        return extra
    end
    if nh_slide_lo <= nh_slide_hi && all(n -> n in block_nhs, nh_slide_lo:nh_slide_hi)
        return Int[]
    end
    fb = default_hybrid_sliding_windows(nx, ny, dx, min_h)
    return filter(w -> !(w in block_nhs) && w >= nh_slide_lo && w <= nh_slide_hi, fb)
end
