...
Test.@testset "Dataset Builder Orchestration Unit Test" begin
    ...
    # `:min_dh`. This synthetic setup should produce trainable rows.
...
Test.@testset "Emitted var_* / tke nonnegative (stress) + parquet round-trip" begin
    dims = (32, 32, 16)
    ...
    mktempdir() do dir
        path = joinpath(dir, "moments_stress.parquet")
        Parquet2.writefile(path, df; compression_codec=:snappy)
        df2 = DataFrames.DataFrame(Parquet2.Dataset(path))
