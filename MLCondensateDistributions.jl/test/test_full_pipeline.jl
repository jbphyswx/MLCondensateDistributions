...
# Integration test: remote GoogleLES → parquet file → DataFrame columns match package schema.
...
Test.@testset "End-to-End GCP Orchestrator Integration Pipeline" begin
    ...
    # Drop stale parquet shards so schema/name checks reflect this run only.
    ...
    target_parquet = joinpath(output_dir, "googleles_case__010__month__01__exp__amip.parquet")
    Test.@test isfile(target_parquet)
    @info "Successfully found serialized payload at: $target_parquet"

    # Validation 2: read the table back with parquet/DataFrames and compare column names to the canonical schema.
    @info "Reconstituting structured representation vector..."
    table = Parquet2.Dataset(target_parquet)
