...
Test.@testset "DatasetBuilderImpl parity and allocation guard" begin
    ...
    # Warmup
    df_old = DatasetBuilder.process_abstract_chunk(fine_fields, metadata, spatial_info, 400f0)
    df_new = DatasetBuilderImpl.process_abstract_chunk_impl(fine_fields, metadata, spatial_info, 400f0)

    Test.@test size(df_old, 1) == size(df_new, 1)

    alloc_old = @allocated DatasetBuilder.process_abstract_chunk(fine_fields, metadata, spatial_info, 400f0)
    alloc_new = @allocated DatasetBuilderImpl.process_abstract_chunk_impl(fine_fields, metadata, spatial_info, 400f0)
...
Test.@testset "DatasetBuilderImpl block_truncated with explicit block_triples" begin
    ...
    df = DatasetBuilderImpl.process_abstract_chunk_impl(fine_fields, metadata, spatial_conv, 400f0)
