...
function maybe_summarize_arrow_dir(dir::String)
    dir = strip(dir)
    isempty(dir) && return
    isdir(dir) || (println("MLCD_PARQUET_DIR is not a directory: ", dir); return)
    try
        paths = sort(filter(f -> endswith(f, ".parquet"), readdir(dir; join=true)))
        isempty(paths) && (println("No .parquet files in ", dir); return)
        # One file is enough for a symptom count
        p = first(paths)
        df = DataFrame(Parquet2.Dataset(p))
        if !hasproperty(df, :var_h)
            println("Parquet file has no column var_h: ", p)
            return
        end
        v = Float64.(df.var_h)
        v = v[isfinite.(v)]
        ...
        nn = count(<(0), v)
        println()
        println("── Existing processed Parquet (symptom only): ", p, " ──")
        println("  var_h: n=$(n), count(<0)=$(nn) ($(n > 0 ? round(100 * nn / n, digits=2) : 0.0)%), min=$(n > 0 ? minimum(v) : NaN)")
        println("  (Parquet rows reflect the **current** builder; this demo shows Zarr-side numerics on raw θ_li.)")
        println("──")
    catch e
        println("Could not scan Parquet dir: ", sprint(showerror, e))
    end
    return nothing
end
