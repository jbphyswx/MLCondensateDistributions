...
Returns 0 when no parquet files exist.
"""
function total_rows(data_dir::String)
    if !isdir(data_dir)
        return 0
    end
    files = MLCD.prune_incompatible_parquet_files!(data_dir, vcat(TRAIN_FEATURE_COLS, TRAIN_TARGET_COLS); verbose=true)
    if isempty(files)
        return 0
    end
    return sum((DataFrames.nrow(DataFrames.DataFrame(Parquet2.Dataset(f))) for f in files); init=0)
end
...
Artifacts:
- Dataset parquet files: `data/processed`
