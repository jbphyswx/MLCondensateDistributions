...
using MLCondensateDistributions.GoogleLES: GoogleLES
...
const DEFAULT_CANDIDATE_SITES = collect(GoogleLES.GoogleLESDefaults.cases) # right now we use vectors inside the functions, so that prevents using the range optimization. TODO: support ntuples, ranges, etc
const DEFAULT_CANDIDATE_MONTHS = collect(GoogleLES.GoogleLESDefaults.months)
const DEFAULT_BATCH_SIZE = 100
const DEFAULT_MIN_DH = GoogleLES.GoogleLESDefaults.min_horizontal_averaging_domain
...
function case_parquet_filename(site_id::Int, month::Int, experiment::String)
    site_id_str = string(site_id, pad=3)
    month_str = string(month, pad=2)
    return "googleles_case__$(site_id_str)__month__$(month_str)__exp__$(experiment).parquet"
end

function case_parquet_path(site_id::Int, month::Int, experiment::String, output_dir::String)
    return joinpath(output_dir, case_parquet_filename(site_id, month, experiment))
end
...
function count_rows(file_path::String)
    ...
    return DataFrames.nrow(DataFrames.DataFrame(Parquet2.Dataset(file_path)))
end

function case_rows(site_id::Int, month::Int, experiment::String, output_dir::String)
    return count_rows(case_parquet_path(site_id, month, experiment, output_dir))
end

function case_is_satisfied(case::NamedTuple; checked_cases, experiment::String, output_dir::String)
    parquet_path = case_parquet_path(case.site_id, case.month, experiment, output_dir)
    # Canonical condition: parquet file exists (non-empty for generated, zero-row for cloudfree).
    return isfile(parquet_path)
end
...
function run_case!(site_id::Int, month::Int; experiment::String, output_dir::String, max_timesteps::Int, timestep_batch_size::Int, min_dh::Float32, include_cfsites::Bool, verbose::Bool, tabular_options::MLCD.TabularBuildOptions)
    started_at = time()
...
function run_case_batch(cases::AbstractVector{<:NamedTuple}; experiment::String, output_dir::String, max_timesteps::Int, timestep_batch_size::Int, min_dh::Float32, include_cfsites::Bool, verbose::Bool, threaded::Bool, tabular_options::MLCD.TabularBuildOptions)
    if threaded
        tasks = [Base.Threads.@spawn run_case!(case.site_id, case.month; experiment=experiment, output_dir=output_dir, max_timesteps=max_timesteps, timestep_batch_size=timestep_batch_size, min_dh=min_dh, include_cfsites=include_cfsites, verbose=verbose, tabular_options=tabular_options) for case in cases]
        return fetch.(tasks)
    end

    results = NamedTuple[]
    for case in cases
        push!(results, run_case!(case.site_id, case.month; experiment=experiment, output_dir=output_dir, max_timesteps=max_timesteps, timestep_batch_size=timestep_batch_size, min_dh=min_dh, include_cfsites=include_cfsites, verbose=verbose, tabular_options=tabular_options))
    end
    return results
end

function run_case_batch_omt(cases::AbstractVector{<:NamedTuple}; experiment::String, output_dir::String, max_timesteps::Int, timestep_batch_size::Int, min_dh::Float32, include_cfsites::Bool, verbose::Bool, tabular_options::MLCD.TabularBuildOptions)
    pkg_path = Base.find_package("OhMyThreads")
    if pkg_path === nothing
        error("PARALLEL_BACKEND=omt requested but OhMyThreads is not available in this environment.")
    end

    OMT = Base.require(Base.PkgId(Base.UUID("67456a42-1dca-4109-a031-0a68de7e3ad5"), "OhMyThreads"))
    return OMT.tmap(cases) do case
        run_case!(case.site_id, case.month; experiment=experiment, output_dir=output_dir, max_timesteps=max_timesteps, timestep_batch_size=timestep_batch_size, min_dh=min_dh, include_cfsites=include_cfsites, verbose=verbose, tabular_options=tabular_options)
    end
end

function run_case_batch_distributed(cases::AbstractVector{<:NamedTuple}; experiment::String, output_dir::String, max_timesteps::Int, timestep_batch_size::Int, min_dh::Float32, include_cfsites::Bool, verbose::Bool, tabular_options::MLCD.TabularBuildOptions)
    if Distributed.nworkers() == 1
        error("PARALLEL_BACKEND=distributed requested but no workers are available.")
    end
    ...
    return Distributed.pmap(cases) do case
        ...
        Main.run_case!(case.site_id, case.month; experiment=experiment, output_dir=output_dir, max_timesteps=max_timesteps, timestep_batch_size=timestep_batch_size, min_dh=min_dh, include_cfsites=include_cfsites, verbose=verbose, tabular_options=tabular_options)
    end
end
...
function batch_generate_data!(;
    ...
    min_dh::Float32 = parse(Float32, get(ENV, "MIN_H_RESOLUTION", string(DEFAULT_MIN_H_RESOLUTION))),
    ...
    for (batch_index, start_idx) in enumerate(1:max(batch_size, 1):length(pending_cases))
        ...
        results = if parallel_backend == :distributed
            run_case_batch_distributed(batch_cases; experiment=experiment, output_dir=output_dir, max_timesteps=max_timesteps, timestep_batch_size=timestep_batch_size, min_dh=min_dh, include_cfsites=include_cfsites, verbose=verbose, tabular_options=tabular_options)
        elseif parallel_backend == :omt
            run_case_batch_omt(batch_cases; experiment=experiment, output_dir=output_dir, max_timesteps=max_timesteps, timestep_batch_size=timestep_batch_size, min_dh=min_dh, include_cfsites=include_cfsites, verbose=verbose, tabular_options=tabular_options)
        else
            run_case_batch(batch_cases; experiment=experiment, output_dir=output_dir, max_timesteps=max_timesteps, timestep_batch_size=timestep_batch_size, min_dh=min_dh, include_cfsites=include_cfsites, verbose=verbose, threaded=(parallel_backend == :threads), tabular_options=tabular_options)
        end
