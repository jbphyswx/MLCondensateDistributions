...
| `min_dh` | Minimum coarse horizontal scale in **meters** (default `<default_obj>.min_horizontal_averaging_domain = 200` for GoogleLES tabular builds). |
| `nh_min` | `max(1, ⌈min_dh / dx_native⌉)` — smallest block width in **cells** that meets the physics minimum. |
| `nh_half` | `⌊N / 2⌋`. |
| `max_dz` | Maximum allowed **mean** vertical layer thickness after coarsening (default `<default_obj>.max_vertical_averaging_domain = 400` m in the tabular path). |
