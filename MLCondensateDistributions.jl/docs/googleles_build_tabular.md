...
| Mode | Meaning |
|------|--------|
| `hybrid` | Default: **lower-band** block `nh` with **prime horizontal tower** + **upper-band** sliding (`sliding_window_budget_h`, default **10**, split 5+5 via `horizontal_sliding_budget_split`) for valid-box sizes in `[max(nh_min,⌊N/2⌋), N]` not already produced as blocks. |
...
Shell / batch scripts: set `MLCD_COARSENING_MODE` to `hybrid`, `block`, or `sliding` only. Removed modes (`convolutional`, `binary`, etc.) **error** at parse time—update shell init if you still export old values.
